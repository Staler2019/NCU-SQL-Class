...
), dept_field_stud_participate AS (
    SELECT student_dept, cf.curriculum_field, student_name
    FROM curriculum_field AS cf
    JOIN enroll USING (semester, course_no)
    JOIN student USING (student_id)
    WHERE select_result = '中選'
    GROUP by student_dept, cf.curriculum_field, student_name
)
SELECT student_dept, curriculum_field, COUNT(*) AS field_persons, (COUNT(*) *1.0 / dept_persons) AS field_rate
FROM dept_field_stud_participate
    JOIN dept_student_view USING (student_dept)
GROUP BY student_dept, curriculum_field
"""
)
...
query = cursor.execute(
    """
WITH course_rank_feedback_view AS (
    SELECT course_name, teacher_name, SUM(feedback_rank) feedback_sum, AVG(feedback_rank) feedback_avg
    FROM course
        JOIN enroll USING (semester, course_no)
        JOIN teach USING (semester, course_no)
        JOIN teacher USING (teacher_id)
...
SELECT course_name, GROUP_CONCAT(teacher_name), feedback_sum, feedback_avg
FROM course_rank_feedback_view
